...
public class CommunityBoardAcademy implements ScriptFile, ICommunityBoardHandler
{
	...
	private void showAcademList(Player player, int page)
	{
		...
		if (!_academyList.contains(player) && player.getLevel() < 40 && player.getLevel() > 4 && player.getClan() == null)
			htmltosend = htmltosend.replace("%mainbutton%", "<button value=\"Register\" action=\"bypass -h _bbsRegAcademyChar\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"><br>");
		if (_academyList.contains(player))
			htmltosend = htmltosend.replace("%mainbutton%", "<button value=\"Unregister\" action=\"bypass -h _bbsUnregisterFromAcademy\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"><br>");
		else
	...
	private void showAcademyChar(Player player, String academyChar, int pageNum)
	{
		...
		for (final Player academy : AcademyList.getAcademyList())
		{
			...
			sb.append("<button value=\"Invite To Academy\" action=\"bypass -h _bbsInviteToAcademy " + academy.getName() + " $items $price\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"><br>");
			sb.append("<button value=\"Back\" action=\"bypass -h _bbsShowAcademyList " + pageNum + "\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"><br>");
			sb.append("</center>");
			break;
		}
