...
public class CommunityClaimDonate implements ICommunityBoardHandler, ScriptFile {
    ...
    private static final String BUTTON = "<button value=\"Get\" action=\"bypass _bbsdonate:get\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">";
    private static final ReentrantLock lock = new ReentrantLock();
    
    @Override
    public String[] getBypassCommands() {
        return new String[] {"_bbsdonate"};
    }
            ...
            else if("get".equals(next))
            {
                ...
                if(count > 0)
                {
                    ...
                    try
                    {
                        ...
                        ps = con.prepareStatement("UPDATE web_donations SET status=2 WHERE obj_id = ? AND status = 1");
    ...
    private int getCoinCount(final int obj_id)
    {
        ...
        try {
            con = DatabaseFactory.getInstance().getConnection();
            ps = con.prepareStatement("SELECT * FROM web_donations WHERE obj_id = ? AND status = 1");
            ps.setInt(1, obj_id);
            rs = ps.executeQuery();
            while(rs.next())
