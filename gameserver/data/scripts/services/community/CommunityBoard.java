...
{
	private static final String[] bbsScriptBypassesFilterWords = {"util", "events","services"};
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if ("bbshome".equals(cmd))
		{
			 ...
			 * try {
					...
					{
						html = html.replace("<?news?>", String.valueOf(FileUtils.readFileToString(new File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/news.txt"), "UTF-8")));
					}
					else
					{
				...
				try {
					...
					{
						html = html.replace("<?gm_event?>", String.valueOf(FileUtils.readFileToString(new File("/lucia/gameserver/data/html-en/scripts/services/communityPVP/pages/events.txt"), "UTF-8")));
					}
					else
					{
...
//		}
		...
		{
			String[] tokens = bypass.split(":");
			if (!StringUtils.startsWithAny(tokens[1].toLowerCase(), bbsScriptBypassesFilterWords)) {
				/*if(AntiScamConfig.EnableSuspicious)
					Log.logSuspicious("Player " + player.getName() + " tried to use bypass " + bypass);*/
				return;
			}
			StringTokenizer st2 = new StringTokenizer(bypass, ";");
