...
public class CommunityRanking implements ScriptFile, ICommunityBoardHandler
{
	...
	{
		PVP("PvP", "pk", "pvpkills", RankingType.NORMAL),
		PK("PK", "pvp", "pkkills", RankingType.NORMAL),
		RAID("Raid Kills", "rk", "raidkills", RankingType.NORMAL),
		EVENT("Event Kills", "event", "eventKills", RankingType.NORMAL),
		SIEGE("Siege Kills", "siege", "siege_kills", RankingType.NORMAL),
		OLY("Olympiad Wins", "oly", "oly_wins", RankingType.NORMAL),
	...
	private void showRanking(Player player, RankingEnum type, int page)
	{
		...
		String html = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "ranking/" + (type.getRankingType() == RankingType.REWARDS ? "rank_rewards" :
			(type.getRankingType() == RankingType.NORMAL ? "rank_normal" : "rank_special") + ".htm"), player);
		...
		html = html.replace("%adenaBypass%", (player.getAccessLevel() > 0 ? "<td align=center><button action=\"bypass _bbsranking:adena\" value=\"Adena\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"></td>" : ""));
	...
	private void showTalesRanking(Player player, RankingEnum type, int page)
	{
		...
		html = html.replace("<?update?>", String.valueOf(TIME_UPDATE / (60 * 1000)));
		html = html.replace("<?last_update?>", TIME_FORMAT.format(_lastUpdate));
		html = html.replace("%adenaBypass%", (player.getAccessLevel() > 0 ? "<tr><td height=40 align=center><button action=\"bypass _bbsranking:adena\" value=\"   Adena\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\"></td></tr>" : ""));
		ShowBoard.separateAndSend(html, player);
	}
	...
	private void loadRankings()
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection())
		{
			for (RankingEnum type : RankingEnum.values())
			{
				...
				{
					try (PreparedStatement statement = con.prepareStatement("SELECT obj_Id,char_name,class_id,clanid," + type.getDbName() + " FROM characters AS c LEFT JOIN character_subclasses AS cs ON (c.obj_Id=cs.char_obj_id) WHERE cs.isBase=1 AND accesslevel = 0 AND " + type.getDbName() + " > 0 ORDER BY " + type.getDbName() + " DESC");
						ResultSet rset = statement.executeQuery())
					{
						while (rset.next())
						{
		...
		// Now we check for the rankings that must have rewards, so we can reward the top 3 chars. If a week has passed from the last rewards given, then we give the new reward for this week
