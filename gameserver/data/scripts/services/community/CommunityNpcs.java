	...
	private static void addNewSubPage(Player player, String raceName)
	{
		...
		for(int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Add " + playerClass.name() + "\" action=\"bypass _bbsAddNewSub_" + playerClass.ordinal() + "\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">";
			}
		}
	...
	private static void changeSubPage(Player player)
	{
		...
		for(int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Change To " + Util.getFullClassName(ClassId.values()[playerClass.getClassId()]) + "\" action=\"bypass _bbsChangeSubTo_" + playerClass.getClassId() + "\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
	...
	private static void cancelSubPage(Player player)
	{
		...
		for(int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Remove " + Util.getFullClassName(ClassId.values()[playerClass.getClassId()]) + "\" action=\"bypass _bbsSelectCancelSub_" + playerClass.getClassId() + "\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
	...
	private static void chooseCertificatePage(Player player)
	{
		...
		for(int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Add " + button[0] + "\" action=\"bypass _bbsscripts:Util:" + button[1] + "\" width=100 height=28 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.Button_DF\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
