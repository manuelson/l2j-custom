...
public class LevelPanel extends Functions
{
	...
	private class AnswerListener implements OnAnswerListener
	{
		...
		public void sayYes()
		{
			...
			if (_player.isInOlympiadMode() || _player.isInOlympiadObserverMode() || _player.isOlympiadCompStart())
			{
				_player.sendMessage("Service not available during you are in Olympiad.");
				return;
			}
			...
			if ((!delevel) && (!Config.SERVICES_LEVEL_UP_ENABLE))
			{
				_player.sendMessage("You can not delevel.");
				return;
			}
			...
			if (Util.getPay(_player, item, count, true))
			{
				...
				//System.out.println("EXP LVL UP: " + exp);
